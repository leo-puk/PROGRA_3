     ...
     public boolean cancelarPedido(int idCompra) {
        ...
        try (Connection conn = DBManager.getInstance().getConnection();
            ...
            try (ResultSet rs = cmdVerificar.executeQuery()) {
                ...
                // Validar plazo de 1 hora (3600 segundos)
                long diferencia = (System.currentTimeMillis() - fechaCompra.getTime()) / 1000;
                if (diferencia > 3600) {
                    throw new RuntimeException("Solo se pueden cancelar pedidos dentro de la primera hora");
                }
                
                if (!"SIN_PAGAR".equals(estadoActual) && !"PAGADO".equals(estadoActual)) {
            ...
            int filas = cmdActualizar.executeUpdate();
