...
public class ClienteDAOImp extends BaseDAOImp<ClienteDTO> implements ClienteDAO{
    ...
    protected String getInsertarQuery() {
        return  "insert into cliente (contraseña, estadoConexion,fechaRegistro,nombreCliente,direccion,email,infoTarjetaCredito,infoCompra, balanceCuenta) values (?,?,?,?,?,?,?,?,?)";
    }

    @Override
    protected String getUpdateQuery() {
       return  "update cliente SET contraseña = ?, estadoConexion = ?,fechaRegistro = ?,nombreCliente = ?,direccion = ?,email = ?,infoTarjetaCredito = ? ,infoCompra = =?, balanceCuenta = ? where idCliente = ?";
    }
    ...
    protected ClienteDTO createFromResultado(ResultSet rs) throws SQLException {
        ...
        p.setIdPersona(rs.getInt("idCliente"));
